...
exec_cmd "kubectl describe rolebinding notes-app-admin-binding -n $namespace"
# Show the permissions the role has
exec_cmd "kubectl get role notes-app-admin -n $namespace -ojson | jq -r '.rules'"
...
# Get the pod name
pod_name=$(kubectl get pods -ojson -l tier=frontend | jq -r '.items[0].metadata.name')
